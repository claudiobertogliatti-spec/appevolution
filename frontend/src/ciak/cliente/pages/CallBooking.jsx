/**
 * Ciak Cliente — /cliente/prenota-call
 * Porting fedele 1:1 da frontend/src/components/cliente/CallBookingPage.jsx
 */
import { useState } from "react";
import { Phone, CheckCircle, ArrowRight, ExternalLink, Sparkles, Clock, MessageSquare, Target } from "lucide-react";
import { clienteFetch, updateClienteUser } from "../api";
...
export function CallBooking({ user, onConfirm }) {
  ...
  const handleConferma = async () => {
    setLoading(true);
    try {
      await clienteFetch("/api/cliente-analisi/call-prenotata", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
      updateClienteUser({ call_prenotata: true });
      setConfermata(true);
  ...
  if (confermata) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-5" style={{ background: C.bg }}>
        ...
        <button
          onClick={() => { window.location.href = "/cliente/analisi-in-preparazione"; }}
