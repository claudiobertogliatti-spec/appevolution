/**
 * Ciak Cliente — /cliente/questionario
 * Porting fedele 1:1 da frontend/src/components/cliente/QuestionarioCliente.jsx
 */
...
export function Questionario({ user, onComplete, onLogout }) {
  ...
  const handleRispostaChange = (campo, value) => {
    setRisposte((prev) => ({ ...prev, [campo]: value }));
    setError(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await clienteFetch("/api/cliente-analisi/questionario", {
        method: "POST",
            ...
            {processSteps.map((step, index) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${step.active ? "" : "opacity-40"}`}
                    style={{
                ...
                {index < processSteps.length - 1 && (
                  <div className="w-16 h-0.5 mt-[-20px]" style={{ background: "#ECEDEF" }} />
                )}
              </React.Fragment>
            ))}
          </div>
