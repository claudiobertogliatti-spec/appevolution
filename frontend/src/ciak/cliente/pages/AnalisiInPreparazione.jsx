/**
 * Ciak Cliente — /cliente/analisi-in-preparazione
 * Porting fedele 1:1 da frontend/src/components/cliente/AnalisiInPreparazione.jsx
 */
import React, { useState, useEffect } from "react";
import { CheckCircle, PlayCircle, ChevronRight, Clock, BookOpen, LogOut, ClipboardList, Target, BarChart3, Calendar, Lightbulb, Video, Megaphone, UserCircle, Handshake, ArrowRight } from "lucide-react";
import { clienteFetch } from "../api";

const MINI_CORSO_MODULI = [
  { id: 1, titolo: "Il Blueprint", descrizione: "La struttura fondamentale per creare un videocorso che funziona e vende.", durata: "8 min", icon: Target },
  { id: 2, titolo: "Argomenti che vendono", descrizione: "Come scegliere argomenti che il mercato vuole davvero acquistare.", durata: "12 min", icon: Lightbulb },
  { id: 3, titolo: "Durata delle lezioni", descrizione: "Quanto devono durare le tue lezioni per massimizzare l'engagement.", durata: "7 min", icon: Clock },
  { id: 4, titolo: "Funnel di vendita", descrizione: "Come costruire un percorso che trasforma visitatori in clienti.", durata: "15 min", icon: BarChart3 },
  { id: 5, titolo: "ADV", descrizione: "Le basi della pubblicità online per promuovere il tuo videocorso.", durata: "11 min", icon: Megaphone },
  { id: 6, titolo: "Profili social", descrizione: "Come ottimizzare i tuoi profili social per attrarre il pubblico giusto.", durata: "9 min", icon: UserCircle },
  { id: 7, titolo: "Non fare tutto da solo", descrizione: "Perché delegare è fondamentale e come costruire il tuo team.", durata: "10 min", icon: Handshake },
];
...
export function AnalisiInPreparazione({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    if (sessionId && userId) {
      clienteFetch(`/api/cliente-analisi/verify-payment?user_id=${userId}&session_id=${sessionId}`, {
        method: "POST",
    ...
    <div className="min-h-screen" style={{ background: "#FAFAF7" }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
            ...
            {processSteps.map((step, index) => (
              <React.Fragment key={step.id}>
                ...
                {index < processSteps.length - 1 && (
                  <div className="w-16 h-0.5 mt-[-20px]" style={{ background: step.status === "completed" ? "#22C55E" : "#ECEDEF" }} />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold mb-4" style={{ background: "#F0FDF4", color: "#166534" }}>
        ...
        <div className="rounded-2xl p-6 mb-8" style={{ background: "#FFFFFF", border: "1px solid #ECEDEF" }}>
          ...
          <div className="space-y-3 mb-4">
            {cosaSucdedeItems.map((item, index) => (
              <div key={index} className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: "#FFD24D20" }}>
          ...
          <div className="mt-4 p-3 rounded-xl text-sm font-medium text-center" style={{ background: "#FFF8DC", color: "#92700C" }}>
        ...
        <div className="rounded-2xl p-6 mb-8" style={{ background: "#FFFFFF", border: "2px solid #FFD24D" }}>
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "#FFD24D15" }}>
              ...
              {MINI_CORSO_MODULI.map((modulo) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ChevronRight className="w-5 h-5 transition-transform group-hover:translate-x-1" style={{ color: "#9CA3AF" }} />
                  </button>
                );
              })}
            </div>
        ...
        <div className="rounded-2xl p-6" style={{ background: "#1E2128" }}>
          ...
          <div className="space-y-3">
            {preparazioneCallItems.map((item, index) => (
              <div key={index} className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: "#FFD24D30" }}>
