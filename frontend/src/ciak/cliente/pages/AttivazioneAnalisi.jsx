/**
 * Ciak Cliente — /cliente/attivazione-analisi
 * Porting fedele 1:1 da frontend/src/components/cliente/AttivazioneAnalisi.jsx
 *
 * NOTA: il checkout Stripe ritorna su `success_url` configurato lato backend.
 * Il backend Evolution attualmente reindirizza al dominio chiamante: verificare
 * che su ciak.io il return `?payment=success` arrivi a /cliente/prenota-call
 * (oppure /cliente/attivazione-analisi e poi reindirizza il guard).
 */
...
export function AttivazioneAnalisi({ user, onLogout }) {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      const response = await clienteFetch(`/api/cliente-analisi/checkout?user_id=${user?.id}`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen" style={{ background: "#FAFAF7" }}>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
            ...
            {analisiIncludes.map((item, index) => {
              ...
              return (
                <div key={index} className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#FFD24D15" }}>
        ...
        <div className="rounded-2xl p-6 mb-8" style={{ background: "#FFF8DC", border: "1px solid #FFD24D40" }}>
        ...
        <div className="text-center">
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-3 px-10 py-4 rounded-xl font-black text-lg uppercase tracking-wide transition-all hover:bg-[#D0D0D0] hover:scale-105 disabled:opacity-50 disabled:hover:scale-100"
