/**
 * Ciak Admin — Parametri Contratto (importata fedelmente da Evolution PRO).
 *
 * Modale per personalizzare i parametri del contratto partner (corrispettivo,
 * royalty, durata, rate). Aperta dal pannello "Controllo Admin" della scheda
 * partner. Chiamate via adminFetch (token admin Ciak).
 */
...
export function ContractParamsModal({ partnerId, partnerName, onClose, onAuthExpired }) {
  ...
  useEffect(() => {
    if (!partnerId) return;
    adminFetch(`/api/admin/partners/${partnerId}/contract-params`)
      .then(r => r.json())
      .then(d => {
        if (d.params) {
          setParams(d.params);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else console.warn("Errore salvataggio:", e);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (e.message === "AUTH_EXPIRED") onAuthExpired?.();
      else console.warn("Errore reset:", e);
    } finally {
      setSaving(false);
    }
  };
...
export default ContractParamsModal;
