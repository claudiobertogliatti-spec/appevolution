/**
 * Ciak Admin — Scheda Partner (importata fedelmente da Evolution PRO).
 *
 * Modale "Centrale Operativa Partner": si apre cliccando sul nome del partner
 * nella Pipeline Partner. Quattro tab — Profilo, Dati Journey, Documenti,
 * Pagamenti — con editing completo dei dati del partner.
 *
 * Tutte le chiamate passano per adminFetch (token admin Ciak). Gli endpoint
 * backend (/api/partners, /api/admin/partner, /api/partner-journey,
 * /api/masterclass-factory, /api/admin/partners/.../documents) sono condivisi
 * con Evolution e restano invariati.
 */
...
function JourneyEditor({ data, saving, saved, onSave, onAuthExpired }) {
  ...
  useEffect(() => {
    ...
    adminFetch(`/api/masterclass-factory/${partnerId}`)
  ...
  }, [partnerId, mcAnswersLoaded, onAuthExpired]);
  ...
  const handleSubmitVideo = async () => {
    ...
    setVideoMsg(null);
    try {
      const res = await adminFetch(`/api/partner-journey/masterclass/submit-video-link`, {
        method: "POST",
  ...
  const handleApproveVideo = async () => {
    ...
    setVideoMsg(null);
    try {
      const res = await adminFetch(`/api/partner-journey/masterclass/approve-video?partner_id=${partnerId}`, {
        method: "POST"
      });
      if (res.ok) {
  ...
  const handleGenerateMcScript = async () => {
    ...
    try {
      // Save answers first
      await adminFetch(`/api/masterclass-factory/${partnerId}/answers`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ answers: mcAnswers })
      });
      // Generate script
      const res = await adminFetch(`/api/masterclass-factory/${partnerId}/generate-script`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ answers: mcAnswers })
      });
      ...
      if (e.message === "AUTH_EXPIRED") onAuthExpired?.();
      else setScriptGenMsg({ ok: false, text: "Errore di rete durante la generazione" });
    } finally {
...
export const PartnerDetailModal = ({ partner, isOpen, onClose, onUpdate, onDelete, onAuthExpired, initialTab = "profilo" }) => {
  ...
  const fetchPayments = async () => {
    ...
    try {
      const res = await adminFetch(`/api/partners/${partner.id}/payments`);
      if (res.ok) {
  ...
  const fetchOnboardingDocs = async () => {
    ...
    try {
      const res = await adminFetch(`/api/admin/partners/${partner.id}/documents`);
      if (res.ok) setOnboardingDocs(await res.json());
    } catch (e) {
      authErr(e);
  ...
  const handleVerifyDoc = async (docType) => {
    ...
    try {
      await adminFetch(`/api/admin/partners/${partner.id}/documents/${docType}/verify`, { method: "PATCH" });
      fetchOnboardingDocs();
    } catch (e) {
      authErr(e);
  ...
  const handleRejectDoc = async () => {
    ...
    try {
      await adminFetch(
        `/api/admin/partners/${partner.id}/documents/${rejectModal.docType}/reject`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const fetchVideoPipeline = async (partnerId) => {
    if (!partnerId) return;
    try {
      const res = await adminFetch(`/api/partner-journey/masterclass/video-status/${partnerId}`);
  ...
  const handleApproveVideo = async () => {
    ...
    setApprovingVideo(true);
    try {
      const res = await adminFetch(`/api/partner-journey/masterclass/approve-video?partner_id=${partner.id}`, { method: "POST" });
      if (res.ok) {
        setVideoPipeline(prev => ({ ...prev, pipeline_status: "approved", video_approved: true }));
      }
    } catch (e) { authErr(e); } finally {
      setApprovingVideo(false);
    }
  };
  ...
  const saveJourneySection = async (section, collection, data) => {
    ...
    try {
      const res = await adminFetch(`/api/admin/partner/${partner.id}/journey`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleUpdatePaymentStatus = async (paymentId, newStatus) => {
    try {
      const res = await adminFetch(`/api/partners/${partner.id}/payments/${paymentId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSegnaPagamentoPartnership = async () => {
    ...
    try {
      const res = await adminFetch(`/api/partners/${partner.id}/segna-pagamento-partnership`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
              ...
              <div className="space-y-6" data-testid="tab-content-profilo">
                ...
                <div className="p-5 rounded-xl border-2 border-dashed" style={{ borderColor: "#F59E0B", background: "#FFFBEB" }}>
                  ...
                  <p className="text-xs mt-3" style={{ color: "#92400E" }}>
                    Le modifiche verranno salvate cliccando "Salva Modifiche"
                  </p>
                </div>
                    ...
                    <div className="p-5 space-y-4" style={{ background: "#FAFAF7" }}>
                      ...
                      {videoPipeline.video_filler_report?.count > 0 && (
                        <p className="text-xs" style={{ color: "#6B7280" }}>
                          Filler rimossi: <strong>{videoPipeline.video_filler_report.count}</strong>
