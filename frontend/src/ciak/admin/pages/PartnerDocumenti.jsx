/**
 * Ciak Admin — Documenti Partner (importata fedelmente da Evolution PRO).
 *
 * Tabella dei partner con lo stato dei tre deliverable strategici:
 * Posizionamento, Script Masterclass, Struttura Corso. "Visualizza" apre un
 * modale a tab con il contenuto completo dei documenti.
 *
 * Sorgenti:
 *  - GET /api/partner-documents/all/summary  → lista riassuntiva
 *  - GET /api/partner-documents/{partner_id} → documenti completi
...
export function PartnerDocumenti({ onAuthExpired }) {
  ...
  const loadSummaries = async () => {
    setLoading(true);
    try {
      const res = await adminFetch(`/api/partner-documents/all/summary`);
      if (!res.ok) throw new Error(`Errore ${res.status}`);
      setSummaries(await res.json());
    } catch (e) {
      if (e.message === "AUTH_EXPIRED") onAuthExpired?.();
      else console.error("Failed to load summaries:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openPartnerDocs = async (partner) => {
    try {
      const res = await adminFetch(`/api/partner-documents/${partner.partner_id}`);
      if (!res.ok) throw new Error(`Errore ${res.status}`);
  ...
  return (
    <div className="p-8 space-y-4" data-testid="partner-documents-view">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Documenti Partner</h1>
          <p className="text-xs text-[#9CA3AF] mt-0.5">Posizionamento, Script e Struttura Corso</p>
        </div>
        <button
          onClick={loadSummaries}
          className="text-xs font-bold px-3 py-1.5 rounded-lg bg-[#FAFAF7] border border-[#ECEDEF] hover:border-[#FFD24D]/30 transition-colors"
        >
          Aggiorna
        </button>
      </div>
          ...
          <div className="divide-y divide-[#ECEDEF]">
            {summaries.length === 0 && (
              <div className="px-5 py-10 text-center text-sm text-[#9CA3AF]">
                Nessun documento partner disponibile.
              </div>
            )}
            {summaries.map((partner) => (
              <div
                key={partner.partner_id}
                className="grid grid-cols-6 items-center hover:bg-[#FAFAF7] transition-colors px-5 py-3"
              >
                <div className="flex items-center gap-3">
...
export default PartnerDocumenti;
