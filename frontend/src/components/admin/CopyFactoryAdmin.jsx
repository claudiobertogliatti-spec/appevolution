      ...
      <div className="bg-gradient-to-br from-[#1a2332] to-[#2c3e55] rounded-xl p-5 border border-pink-500/20">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-pink-500 to-purple-500 flex items-center justify-center">
            <Edit3 className="w-6 h-6 text-[#0F172A]" />
          </div>
          <div className="flex-1">
            <h2 className="text-lg font-extrabold text-[#0F172A]">STEFANIA Copy Factory — Admin Review</h2>
            <p className="text-sm text-[#5F6572]">Editing finale delle Masterclass prima dell'approvazione Partner</p>
          </div>
          <div className="flex gap-2">
            <button 
              onClick={() => setShowRules(!showRules)}
              className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors ${showRules ? "bg-pink-500 text-[#0F172A]" : "bg-[#FAFAF7] border border-[#ECEDEF] text-[#5F6572] hover:border-pink-500/30"}`}
            >
              <BookOpen className="w-4 h-4" /> 10 Regole d'Oro
            </button>
            <button 
              onClick={() => setShowCases(!showCases)}
              className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors ${showCases ? "bg-purple-500 text-[#0F172A]" : "bg-[#FAFAF7] border border-[#ECEDEF] text-[#5F6572] hover:border-purple-500/30"}`}
            >
            ...
            {goldenRules.map(rule => (
              <div key={rule.num} className="bg-[#FAFAF7] border border-[#ECEDEF] rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-bold text-[#0F172A]">{rule.title}</span>
                </div>
            ...
            {successCases.map(c => (
              <div key={c.id} className="bg-[#FAFAF7] border border-[#ECEDEF] rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-[#0F172A]">{c.partner_name}</span>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1">
                      <div className="font-bold text-sm text-[#0F172A]">{script.partner_info?.name || "Partner"}</div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border border-pink-500/30 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-extrabold text-[#0F172A]">{selectedScript.partner_info?.name}</div>
                  ...
                  {Object.entries(BLOCK_LABELS).map(([blockId, label]) => (
                    <div key={blockId} className="p-4">
                      ...
                      <textarea
                        ...
                        className="w-full bg-[#FAFAF7] border border-[#ECEDEF] rounded-lg px-4 py-3 text-sm text-[#0F172A] placeholder:text-[#9CA3AF] outline-none focus:border-pink-500/50 transition-colors resize-none min-h-[100px]"
                      />
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white border border-[#ECEDEF] rounded-xl p-4">
                ...
                <textarea
                  ...
                  className="w-full bg-[#FAFAF7] border border-[#ECEDEF] rounded-lg px-4 py-3 text-sm text-[#0F172A] placeholder:text-[#9CA3AF] outline-none focus:border-pink-500/50 transition-colors resize-none min-h-[80px]"
                />
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={saving}
                  className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-pink-500 to-purple-500 text-[#0F172A] rounded-xl px-6 py-3 text-sm font-extrabold hover:opacity-90 transition-opacity disabled:opacity-50"
                >
