    ...
    <div className="h-full flex flex-col bg-white rounded-xl border border-[#ECEDEF] overflow-hidden" data-testid="andrea-chat">
      ...
      <div className="p-4 border-b border-[#ECEDEF] bg-gradient-to-r from-blue-500/20 to-cyan-500/20">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-[#0F172A] font-bold text-sm shadow-lg">
            AN
          </div>
          <div>
            <div className="text-sm font-extrabold text-[#0F172A] flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full">
                Produzione Contenuti & Video Coach
              </span>
            </div>
          ...
          <div key={i} className={`flex items-start gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0
              ${msg.role === "user" 
                ...
                : "bg-gradient-to-br from-blue-500 to-cyan-500 text-[#0F172A]"}`}
            >
      ...
      <div className="p-4 border-t border-[#ECEDEF] bg-[#FAFAF7]">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-[#FAFAF7] border border-[#ECEDEF] rounded-xl px-4 py-3 text-sm text-[#0F172A] placeholder:text-[#9CA3AF] outline-none focus:border-blue-500/50 transition-colors"
            disabled={loading}
          />
          <button
            onClick={sendMessage}
            disabled={loading || !input.trim()}
            className="bg-gradient-to-r from-blue-500 to-cyan-500 text-[#0F172A] rounded-xl px-4 py-3 font-bold text-sm hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
          >
