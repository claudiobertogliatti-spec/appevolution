...
export function EmailAutomation({ partner }) {
  ...
  const loadData = async () => {
    ...
    try {
      const [automationsRes, templatesRes, queueRes] = await Promise.all([
        fetch(`${API}/api/email-automation/partner/${partnerId}`),
        fetch(`${API}/api/email-automation/templates`),
        fetch(`${API}/api/email-queue/${partnerId}`)
      ]);
  ...
  const toggleAutomation = async (id) => {
    try {
      const response = await fetch(`${API}/api/email-automation/${id}/toggle`, {
        method: 'PUT'
      });
      if (response.ok) {
        loadData();
  ...
  const deleteAutomation = async (id) => {
    ...
    try {
      const response = await fetch(`${API}/api/email-automation/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        loadData();
  ...
  const generateAISequence = async () => {
    ...
    try {
      const response = await fetch(`${API}/api/email-automation/generate-sequence?partner_id=${partnerId}&partner_name=${encodeURIComponent(partnerName)}&partner_niche=${encodeURIComponent(partnerNiche)}&sequence_type=nurture`, {
        method: 'POST'
      });
  ...
  const createFromTemplate = async (templateKey) => {
    ...
    try {
      const response = await fetch(`${API}/api/email-automation/create`, {
        method: 'POST',
