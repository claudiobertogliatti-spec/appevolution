...
export function DomainConfiguration({ partner, isAdmin = false, onNavigate }) {
  ...
  const loadDomainRequest = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API}/api/domain/partner/${partnerId}`);
      if (response.ok) {
  ...
  const handleSubmitDomain = async () => {
    ...
    try {
      const response = await fetch(`${API}/api/domain/request`, {
        method: 'POST',
  ...
  const handleAddDnsParams = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`${API}/api/domain/${domainRequest.id}/dns-params`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleMarkActive = async () => {
    setIsSubmitting(true);
    try {
      const response = await fetch(`${API}/api/domain/${domainRequest.id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
