  ...
  return (
    <div className="flex flex-col gap-2 mx-2 mb-2">
      <button
        ...
        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-extrabold transition-all
          ${adminUser === "claudio" 
            ...
            : 'bg-[#ECEDEF] text-[#5F6572] hover:bg-white/15 hover:text-[#0F172A]'}`}
      ...
      <button
        ...
        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-extrabold transition-all
          ${adminUser === "antonella" 
            ? 'bg-purple-500 text-[#0F172A] shadow-md' 
            : 'bg-[#ECEDEF] text-[#5F6572] hover:bg-white/15 hover:text-[#0F172A]'}`}
        data-testid="admin-switch-antonella"
      >
        <span className="w-7 h-7 rounded-full bg-purple-600 text-[#0F172A] flex items-center justify-center text-[10px] font-bold flex-shrink-0">
          AR
        </span>
