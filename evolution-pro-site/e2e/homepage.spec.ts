...
test('reduced motion mantiene visibili i contenuti principali', async ({ page }) => {
  ...
  // scelta di prodotto: i marquee decorativi (loghi) continuano a scorrere anche con reduced motion
  for (const selector of ['.collaborations__track', '.direction-tools__track']) {
    expect(await page.locator(selector).first().evaluate((el) => getComputedStyle(el).animationName)).toBe('marquee-scroll');
  }
});
