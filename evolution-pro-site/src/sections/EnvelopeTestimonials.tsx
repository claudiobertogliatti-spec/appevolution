...
function TestimonialEnvelope({ testimonial, onOpen, index }: { testimonial: Testimonial; onOpen: () => void; index: number }) {
  ...
  return (
    ...
    <article className="envelope envelope--cinematic" data-testid="testimonial-envelope" data-mobile-state="final">
      <motion.div className="envelope__flap" data-flap-color="yellow" aria-hidden="true" data-testid="testimonial-flap" initial={{ rotateX: 0 }} animate={{ rotateX: [0, 0, 180, 180, 180, 0] }} transition={autoplayTransition(delay, [0, .1, .24, .82, .92, 1])} />
      <motion.div className="envelope__seal" data-seal-style="gray-spiral" data-testid="testimonial-seal" initial={{ scale: 1, rotate: 0 }} animate={{ scale: [1, 1, 0, 0, 0, 1], rotate: [0, 0, -18, -18, 0, 0] }} transition={autoplayTransition(delay, [0, .08, .18, .82, .93, 1])}><img className="envelope__seal-logo" src="/brand/evolution-spiral-gray.webp" alt="Spirale Evolution PRO" /></motion.div>
      <motion.div className="envelope__letter" data-testid="testimonial-letter" initial={{ y: 170, rotate: -4 }} animate={{ y: [170, 170, -132, -132, 170], rotate: [-4, -4, 0, 0, -4] }} transition={autoplayTransition(delay, [0, .2, .38, .84, 1])}>
        ...
        <motion.div className="envelope__actions" data-testid="testimonial-actions" initial={{ opacity: 0, y: 16 }} animate={{ opacity: [0, 0, 1, 1, 0], y: [16, 16, 0, 0, 10] }} transition={autoplayTransition(delay, [0, .46, .62, .9, 1])}>
          ...
          <span className="button button--primary">Guarda la testimonianza</span>
        </motion.div>
      </motion.div>
      <div className="envelope__front" aria-hidden="true" />
      <button className="envelope__video-trigger" aria-label={`Guarda la testimonianza di ${testimonial.name}`} onClick={onOpen} />
    </article>
    </div>
  );
}
