...
_ADVANCED_PHASES = {"F3", "F4", "F5", "F6", "F7", "F8", "F9", "LIVE", "OTTIMIZZAZIONE"}


def _migrate_progress_from_phase(phase: Optional[str]) -> tuple[dict, dict]:
    """
    ...
    Returns:
        (progress_data, audit_info)
        audit_info keys: mapped_state, fallback_used, lossy_fields
    """
    fallback_used = phase is None or phase not in _PHASE_TO_STATE
    current_state = _PHASE_TO_STATE.get(phase, "ONBOARDING") if phase else "ONBOARDING"
    progress = _build_default_progress()
    lossy_fields: list[str] = []

    for state in STATE_ORDER:
        if state == current_state:
            # Active state — steps start from false (lossy for counter fields)
            if state == "VIDEOCORSO":
                lossy_fields.append(
                    "VIDEOCORSO.MODULES_TOTAL reset to 0 — partner must re-enter module count"
                )
                lossy_fields.append(
                    "VIDEOCORSO.MODULES_UPLOADED reset to 0 — upload progress not recoverable"
                )
            break
        # Mark completed states as fully done
        ...
        for k, v in state_canon.items():
            if isinstance(v, bool):
                ...
                progress[state][k] = 5   # assumed reasonable default for completed state
                lossy_fields.append(
                    f"{state}.MODULES_TOTAL assumed 5 (state already complete, value not critical)"
                )
...
class StefaniaEngine:
    ...
    @staticmethod
    def evaluate(partner: dict, migration_source: str = "lazy") -> dict:
        """
        Full evaluation of partner guided state.
        Returns updated guided sub-document (does NOT persist).

        migration_source: "lazy" (first access via API) | "bulk" (migration script)
          Only meaningful on first initialization — ignored on subsequent calls.
        ...
        if not guided:
            # First time: initialize from existing phase with full audit trail
            phase = partner.get("phase")   # intentionally None if missing
            progress_data, audit_info = _migrate_progress_from_phase(phase)
            current_state = audit_info["mapped_state"]
            fallback_used = audit_info["fallback_used"]
            review_recommended = fallback_used or phase in _ADVANCED_PHASES

            if fallback_used:
                logger.warning(
                    "[STEFANIA] Migration: partner=%s phase=%r is missing or unrecognized — "
                    "defaulting to ONBOARDING. Set migration_review_required=True.",
                    partner.get("id", "<no-id>"), phase
                )
            ...
            guided = {
                ...
                # ── Migration audit fields (set once, never overwritten) ──────
                "migration_source": migration_source,
                "migration_at": datetime.now(timezone.utc).isoformat(),
                "migration_legacy_phase": phase,
                "migration_fallback_used": fallback_used,
                "migration_lossy_fields": audit_info["lossy_fields"],
                "migration_review_recommended": review_recommended,
            }
