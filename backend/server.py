...
async def get_videos_for_review(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    for doc in masterclass_docs:
        ...
        result.append({
            ...
            "review_url": doc.get("video_review_url") or doc.get("video_gcs_review_url"),
        ...
        for lesson_id, lesson in lessons.items():
            ...
            if lesson.get("pipeline_status") and lesson.get("pipeline_status") is not None:
                ...
                result.append({
                    ...
                    "review_url": lesson.get("video_review_url") or lesson.get("video_gcs_review_url"),
...
async def admin_video_pipeline_config(
    ...
    cfg = {
        ...
        "video_pipeline": {
            "version": "clean-only-2026-04-28-v3",
            "cut_padding_s": 0.30,
            "cut_codec": "libx264 crf 20 single-pass filter_complex trim+atrim+concat (audio sync robusto)",
            "cut_fallback": "per-segment -c copy + concat se single-pass fallisce/timeout",
            "video_enhance_enabled": _present("VIDEO_ENHANCE_ENABLED"),
            "video_enhance_note": "Sub burn-in, Remotion intro/outro, Shotstack DALL-E/music/zoom/overlay sono opt-in via VIDEO_ENHANCE_ENABLED=true. Default disabilitato: editing creativo manuale a valle.",
        },
    }
    return cfg
