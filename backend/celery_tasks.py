...
def check_partner_journey_solleciti(self):
    """
    Sollecito motivazionale del percorso operativo (deadline 21 giorni).

    Gira ogni giorno. Per ogni partner con journey attivo calcola i giorni
    trascorsi dall'attivazione (payment_at/contract_signed_at dello step
    01-contratto). Alle soglie G7 / G14 / G18, se il milestone atteso non è
    ancora completato, invia un sollecito motivazionale (voce Claudio) e lo
    registra con data in `ciak_partnership_emails`.
    ...
    try:
        async def _run():
            from services.ciak_partnership_email import send_sollecito_async, set_db as _set_email_db
            ...
            inviati = 0
            # Partner con journey avviato = step 01-contratto completato (attivazione)
            ...
            for a in anchors:
                ...
                if giorni < SOGLIE[0][0]:
                    continue  # ancora dentro la prima settimana

                # Se ha già lanciato, nessun sollecito
                ...
                # Soglia massima raggiunta dal giorno corrente
                ...
                # È davvero in ritardo? Lo step atteso non è ancora done
                ...
                if step_doc and step_doc.get("status") == "done":
                    continue  # in pari / avanti su questa soglia: niente sollecito

                # Idempotenza: soglia già sollecitata con successo (o invio in corso)?
                ...
                # Prossima azione = primo step non completato (focus singolo)
