"""
Raccolta dati del briefing di Luca — FONTE DI VERITA'.

⛔ Sta in `backend/` e non in `scripts/` per una ragione operativa precisa: il
deploy builda con `gcloud run deploy --source ./backend`, quindi **solo questa
cartella finisce nel container**. Il 2/9/2026 il briefing delle 7:45 e' partito
davvero (Celery Beat funziona) ed e' morto su `No module named 'briefing_luca'`,
perche' il task importava da `scripts/` -- che nell'immagine non esiste.

`scripts/briefing_luca.py` resta come interfaccia a riga di comando e importa da
qui: una sola implementazione, due modi di invocarla.
"""
#!/usr/bin/env python3
"""
