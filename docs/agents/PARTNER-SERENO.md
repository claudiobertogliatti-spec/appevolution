...
**5.3 Servizi rivestito:** `SerenoServizi` è ora la vetrina sereno completa (4 gruppi, 12 servizi con nome + prezzo + "Ideale se", pulsante "Scopri" → dettaglio). Prezzi/nomi importati da `BOOSTER_CATALOG` reale (una fonte; Avatar 1.490€, Video Premium 590€, Copywriting 497€, Campagne 348€/mese ecc.) — nulla inventato. Scartati gli stock photo Unsplash del legacy (design-lead: no foto generiche). `BoosterEvoPage` col flag rende la vetrina sereno; il **Dettaglio con checkout Stripe (`request()`) resta INTATTO**, raggiunto via `onOpen`. Preview: la route importa il catalogo reale; fix build.cjs con DefinePlugin (feature.js legge `process.env` al top-level → serviva definirlo nel bundle preview). Verificato via DOM: 4 sezioni, 12 card, prezzi reali, zero overflow.
